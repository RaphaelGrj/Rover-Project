# ROVER --- Archive détaillée du journal d'avancement

> **Ce fichier est volumineux et n'est pas destiné à être relu
> intégralement en début de session.** Il contient l'historique complet,
> session par session, avec tout le détail de diagnostic/investigation
> (mesures, hypothèses écartées, méthode). Consultez-le seulement pour
> retrouver le contexte fin d'une décision ou d'un test passé précis.
>
> Pour une reprise normale de session : voir `PROGRESS.md` (résumé court,
> état actuel + prochaines étapes). Référence architecturale : voir
> `ARCHITECTURE_AND_ROADMAP.md`. Spec du protocole : voir
> `ROVER_PROTOCOL.md`.
